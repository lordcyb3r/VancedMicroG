...
package app.revanced.android.gms.chimera.container;
...
import app.revanced.android.gms.dynamite.IDynamiteLoader;
...
public class DynamiteLoaderImpl extends IDynamiteLoader.Stub {
    ...
    public int getModuleVersion2NoCrashUtils(IObjectWrapper wrappedContext, String moduleId, boolean updateConfigIfRequired) throws RemoteException {
        ...
        try {
            return Class.forName("app.revanced.android.gms.dynamite.descriptors." + moduleId + ".ModuleDescriptor").getDeclaredField("MODULE_VERSION").getInt(null);
        } catch (Exception e) {
            Log.w(TAG, "No such module known: " + moduleId);
        }

        if (moduleId.equals("app.revanced.android.gms.googlecertificates")) {
            return app.revanced.android.gms.dynamite.descriptors.app.revanced.android.gms.googlecertificates.ModuleDescriptor.MODULE_VERSION;
        }
        if (moduleId.equals("app.revanced.android.gms.cast.framework.dynamite")) {
            Log.d(TAG, "returning temp fix module version for " + moduleId + ". Cast API wil not be functional!");
            return 1;
        }

        if (moduleId.equals("app.revanced.android.gms.maps_dynamite")) {
